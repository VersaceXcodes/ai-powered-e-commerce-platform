...
const { PORT = 3000, DATABASE_URL, PGHOST, PGDATABASE, PGUSER, PGPASSWORD, PGPORT = 5432, JWT_SECRET = 'super-secret-key', FRONTEND_URL = 'http://localhost:5173' } = process.env;
// POSTGRES
...
io.on('connection', (socket) => {
    if (socket.user) {
        ...
        if (socket.user.role === 'vendor')
            socket.join(`vendor:${socket.user.user_id}`);
    }
    if (socket.cart_id)
        socket.join(`cart:${socket.cart_id}`);
});
// -- UTILS --
...
app.post('/auth/register', async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO users (user_id, name, email, password_hash, role, profile_image_url, is_blocked, created_at, updated_at)
...
app.post('/auth/password/forgot', async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO password_reset_tokens (reset_token, user_id, expires_at, used, created_at)
      VALUES ($1, $2, $3, $4, $5)`, [reset_token, user_id, expires_at, false, created_at]);
...
app.post('/auth/password/reset', async (req, res) => {
    try {
        ...
        await pool.query("UPDATE users SET password_hash = $1, updated_at = $2 WHERE user_id = $3", [password_hash, nowISO(), tok.user_id]);
        // Set token as used
...
app.patch('/users/:user_id', authenticateToken, async (req, res) => {
    try {
        ...
        if (input.is_blocked !== undefined) {
            emitRoom('user.block_status.changed', `user:${user_id}`, { user_id, is_blocked: updated.is_blocked });
            if (updated.role === 'admin')
                emitRoom('user.block_status.changed', 'admins', { user_id, is_blocked: updated.is_blocked });
        }
        res.json(updated);
    }
    catch (err) {
